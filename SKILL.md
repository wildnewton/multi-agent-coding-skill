...
- **Executor (`run_codex.py`):** owns deterministic handoff/state/audit mechanics, mechanical gate enforcement, and mechanical guarding/recording of Hermes-side required external verification.
- **Hermes:** transports user intent/answers, performs remaining branch/commit/push/CI/PR/approved-merge mechanics, and provides the practical host/environment capability for required external-verification execution outside Codex agents.
...
- When the Task-Review-certified acceptance criteria require one real live/external gate, Coordinator may return `VERIFY_EXTERNAL` with one non-destructive command, boundary, and reason. Executor accepts it as `Coordinator -> Executor`; Hermes finishes the commit/push/PR-HEAD bridge and then invokes the pending Executor action in the Hermes-capable host/environment. Executor validates/records the run and reverses the same pending to `Executor -> Coordinator`. Non-zero/timeout/command-execution failure is evidence for Coordinator classification, not automatic regression. Executor/orchestration failure remains workflow `ERROR` and leaves `Coordinator -> Executor` ownership unresolved.
- A Codex agent's inability to exercise the requested boundary is not by itself a reason to ask the User. Hermes should first attempt the Coordinator-requested run through the Executor action when Hermes has sufficient capability and information. Coordinator may use the existing `AWAIT_USER_DECISION` required-action fallback only when a Hermes-side attempt, or other decisive existing environment/capability evidence, establishes that Hermes cannot exercise the boundary or lacks required access/capability. Returned evidence is preserved with `externally_supplied` provenance; it is not mechanically attested. Do not create a remote-runner/environment-provisioning framework in this MVP.
...
- Preserved external-verification raw evidence is workflow context for Coordinator/Review, not public audit content. GitHub audit/handoff traces for external-verification results use a metadata-only projection and must not reproduce captured stdout/stderr or externally supplied raw evidence.
...
Pending required external verification is a Hermes-side mechanical Executor action, not an agent invocation:
...
A `Coordinator -> Executor` required external-verification request reuses the same ACCEPT/BRIDGE ownership pattern but ends in a Hermes-side mechanical action instead of invoking Codex: after Hermes has committed/pushed/synchronized the candidate, Hermes invokes `--run-external-verification` in the capable host/environment. Executor validates current HEAD/PR HEAD, publishes the accepted handoff trace, runs the command without semantic interpretation, persists the current evidence checkpoint, and reverses pending to `Executor -> Coordinator`. When that result handoff is later audited, only safe metadata is published; raw captured evidence remains in the workflow checkpoint supplied to Coordinator/Review.
...
3. **GREEN -> required external verification when applicable -> Review -> Coordinator**  
   Coordinator implements the smallest GREEN. If the certified acceptance criteria require one live/external gate, Coordinator requests it only against a committed candidate: normally return `VERIFY_EXTERNAL`, let Hermes finish the commit/push/PR-HEAD bridge, then Hermes invokes the pending Executor action in its available host/environment. A Codex agent's inability to run the boundary does not trigger user escalation by itself. Coordinator consumes the resulting evidence and semantically classifies failures. Only when a Hermes-side attempt, or other decisive capability evidence, establishes that Hermes cannot exercise the boundary may Coordinator instead use structured `AWAIT_USER_DECISION` once the exact candidate HEAD is already committed/current; the returned evidence is preserved as externally supplied evidence. Testing is re-entered only if classification exposes Testing-owned harness/coverage work or focused RED is required. Any fix that changes HEAD makes earlier external evidence stale and requires rerun.
...
4. **User decision / required action**  
   ...
   When `AWAIT_USER_DECISION` carries structured external-verification metadata, decisive evidence has established that Hermes itself cannot execute the requested boundary, so the request is for one exact non-destructive run in another capable environment or for user direction. The returned answer is still persisted as the exact User -> Coordinator answer and is additionally preserved as the current `externally_supplied` verification checkpoint. Coordinator and Review must validate its sufficiency/provenance; Hermes must not interpret it semantically.
