...
- Classify test-looking runs by purpose, not command syntax/name. Ordinary deterministic targeted/full tests may remain bridge mechanics; an ad-hoc live diagnostic may use otherwise permitted diagnostic mechanics but is diagnostic evidence only. Once a live/external run is required for acceptance, Review, or merge readiness, Coordinator must use `VERIFY_EXTERNAL`, and direct ordinary terminal output cannot satisfy that gate. A Testing sandbox limitation on an already-existing suite does not make Testing the mechanical runner; if no Testing-owned test/harness work remains and required external evidence is still needed, Coordinator routes it through `VERIFY_EXTERNAL`.
