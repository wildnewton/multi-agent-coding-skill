...
5. If the certified task requires one live/external verification gate, return `VERIFY_EXTERNAL` for the committed candidate HEAD. Hermes attempts the non-destructive run through the Executor before involving the user; a Codex agent's inability alone is not grounds for user escalation. Interpret the result as evidence and distinguish current-change regression, invalid test/harness behavior, pre-existing production/config defect, and environment/external-service failure. Only current-change regression is automatically blocking; invalid/inconclusive evidence does not satisfy the gate. If decisive evidence shows Hermes cannot perform the run, use `AWAIT_USER_DECISION` with structured external-verification metadata and treat the returned evidence as externally supplied, not mechanically attested.
...
When decisive evidence shows Hermes cannot perform required external verification, additionally include:

`"external_verification":{"command":"<exact non-destructive command>","boundary":"<required external boundary>","reason":"<why external execution/direction is needed>","expected_head":"<current committed candidate HEAD>"}`
