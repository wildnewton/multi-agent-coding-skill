#!/usr/bin/env python3
"""Opt-in smoke harness for long-running Codex invocation survival."""
...
print(json.dumps({{"type":"thread.started","thread_id":"long-running-invocation-smoke"}}))
print(json.dumps({{
    "type":"item.completed",
    "item":{{
        "type":"agent_message",
...
def main(argv=None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="long-running-invocation-smoke-") as temp:
        ...
        git(repo, "config", "user.name", "Long Running Invocation Smoke")
